...
class TestObject(unittest.TestCase):
            ...
            for key, value in self.PARAMS:
                self.assertEqual(
                    ...
                    "Key({0}) orig({1}) vs new({2})".format(
                        key, getattr(orig, key), getattr(new, key)
                    ),
                )
    ...
    def test_childtypes_is_same(self):
        ...
        for x in old.CHILDTYPES:
            self.assertTrue(x in new.CHILDTYPES, "CHILDTYPE {0} missing".format(x))
    ...
    def test_has_the_same_functions(self):
        ...
        funcs = [
            x
            for x in dir(old)
            if callable(getattr(old, x)) and x not in ignore_functions
        ]
        for x in funcs:
            self.assertTrue(
                hasattr(new, x) and callable(getattr(new, x)),
...
class TestVersionedObject(unittest.TestCase):
    ...
    def test__get_versioned_value_gets_newer_value_with_multiple_exact_version_profiles(
        self,
    ):
        self._check((1, 0, 0), self.DEFAULT_VALUE)

    def test_add_profile_raises_error_on_adding_lower_version_after_adding_a_higher_version(
        self,
    ):
