...
def addActor(PAN, key, VSYS, ACTOR, BADACTORS):
    ...
    panReq = urllib2.Request(
        'https://' + PAN + '//api/?type=config&action=set&key=' + key + '&xpath=/config/devices/entry/vsys/entry[@name=' + "'" + VSYS + "'" + ']/address/entry[@name=' + "'" + ACTOR + "'" + ']&element=<ip-netmask>' + ACTOR + '/32</ip-netmask>')
    req = opener.open(panReq)
...
def remActor(PAN, key, VSYS, ACTOR, BADACTORS):
    ...
    panReq = urllib2.Request(
        'https://' + PAN + '//api/?type=config&action=delete&key=' + key + '&xpath=/config/devices/entry/vsys/entry[@name=' + "'" + VSYS + "'" + ']/address-group/entry[@name=' + "'" + BADACTORS + "'" + ']&element=<member>' + ACTOR + '</member>')
    req = opener.open(panReq)
...
def main(argv=sys.argv):
    ...
    parser.add_option('-a', '--add', dest="ACTION", default="0",
                      help="Adding or removing an address object. Acceptable options are add or rem")
